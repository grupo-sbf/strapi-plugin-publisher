...
/**
 * Builds a mock strapi whose plugin service registry returns the provided
 * service stubs, and captures the cron task registered by registerCronTasks.
 */
function setup({ records, toggleImpl }) {
	...
	// getPluginService resolves services off the global strapi.
...
test('processes toggles sequentially (awaits each before the next)', async () => {
	// First toggle is slow; if toggles ran concurrently the order would
	// interleave (start:1, start:2, ...). Sequential awaiting yields fully
...
test('a failing toggle does not abort the rest of the batch', async () => {
	...
	// id 1 started and threw (no end), id 2 still ran to completion.
...
test('skips a tick while a previous run is still draining', async () => {
	...
	// Fire two ticks concurrently; the second must return early (guard).
